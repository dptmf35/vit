...
class YOLO11DatasetCollector(Node):
    def __init__(self):
        super().__init__('yolo11_dataset_collector')
        ...
        if self.test_mode:
            self.get_logger().info("🔍 YOLO11 Dataset Collector initialized - TEST MODE")
            self.get_logger().info("Detection results will be published to /yolo11_detection_rviz")
            self.get_logger().info("NO DATA COLLECTION will occur")
        else:
            self.get_logger().info("YOLO11 Dataset Collector initialized - DATA COLLECTION MODE")
        ...
        self.get_logger().info("ROS2 Service: ros2 service call /toggle_yolo11_collection_mode std_srvs/srv/SetBool \"{data: true}\"")
        self.get_logger().info("Status Topic: ros2 topic echo /yolo11_collector_mode_status")
        self.get_logger().info("================================")
...
"""
    ...
    def image_callback(self, msg):
        try:
            ...
            # Run YOLO11 prediction with trained model
...
def main(args=None):
    ...
    node = YOLO11DatasetCollector()
    
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        if node.test_mode:
            node.get_logger().info("🔍 Shutting down YOLO11 Dataset Collector (TEST MODE)...")
        else:
            node.get_logger().info("Shutting down YOLO11 Dataset Collector...")
